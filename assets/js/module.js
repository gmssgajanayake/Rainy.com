...
export const getDate = () => {
    ...
    return `${nameOfDay} ${date.getDate()}, ${nameofMonth}`;
};
...
export const getTime = (unixTime) => {
    ...
    return (data.getHours() > 12 ? data.getHours() - 12 : data.getHours()) + ":" + data.getMinutes();
};
...
const setCurrentWeatherData = (data) => {
    countryName.forEach(country => {
        if (country.code === data.sys.country) {
            updateWeatherData(data, country.name);
            return true;
        }
    });
};
...
export const getAirPollutionData = (lat, lon) => {
    fetchData(url.airPollution(lat, lon), data => {
        updateAirPollutionData(data);
    });
};

export const getFiveDayForecast = (lat, lon) => {
    fetchData(url.fiveDayForecast(lat, lon), data => {
        updateFiveDateForecast(data);
    });
}
